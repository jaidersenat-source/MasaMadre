  ...
  <div>
    <div class="flex items-center gap-2 mb-1.5">
      ...
      <span class="font-mono text-xs uppercase tracking-widest"
            style="color:var(--color-trigo);opacity:.7">Mi panadería</span>
    </div>
    <h1 class="font-display font-bold leading-tight"
        style="font-size:clamp(1.5rem,5vw,1.9rem);color:var(--color-corteza)">
      {{ $panaderia->nombre }}
    </h1>
    <p class="text-sm mt-0.5" style="color:var(--color-corteza);opacity:.4">
      {{ $panaderia->ciudad }} &nbsp;·&nbsp; {{ $panaderia->regional }}
    </p>
  </div>

  @if(!$procesoActivo)
    <a href="{{ route('panaderia.proceso.create') }}" class="pan-btn pan-btn-primary w-full sm:w-auto justify-center">
...
════════════════════════════════════════ --}}
@if($procesoActivo)
@php $progreso = $procesoActivo->progreso(); @endphp
...
<div class="pan-card mb-5" style="border-left:3px solid var(--color-trigo)">
  ...
  <div class="px-5 py-4 border-b flex flex-wrap items-center justify-between gap-3"
       style="border-color:rgba(107,66,38,.1)">
    <div class="flex items-center gap-3">
      ...
      <div class="relative shrink-0">
        ...
        <div class="absolute inset-0 rounded-full animate-ping"
             style="background:var(--color-trigo);opacity:.4"></div>
      </div>
      <div>
        <div class="flex items-center gap-2">
          <span class="pan-badge pan-badge-amber">En proceso</span>
          <span class="font-mono text-xs" style="color:var(--color-corteza);opacity:.35">
            #{{ $procesoActivo->id }}
          </span>
        </div>
        <h2 class="font-display font-semibold mt-0.5"
          ...
          Proceso activo
          <span class="font-sans font-normal text-sm" style="color:var(--color-corteza);opacity:.4">
  ...
  <div class="p-5">
    ...
    <div class="flex items-center justify-between mb-2">
      <span class="text-xs" style="color:var(--color-corteza);opacity:.45">Progreso del proceso</span>
      <span class="font-mono text-xs font-medium" style="color:var(--color-trigo)">{{ $progreso }}%</span>
    </div>
    <div class="h-1.5 rounded-full overflow-hidden mb-6"
         style="background:rgba(107,66,38,.1)">
    ...
    {{-- Timeline de días ── 5 días + Pan ── --}}
    <div class="-mx-1 overflow-x-auto pb-1">
    <div class="flex items-start gap-2 min-w-max px-1">

      @for($d = 1; $d <= 5; $d++)
        @php $diaReg = $procesoActivo->dias->firstWhere('dia', $d); $esSiguiente = $d === $procesoActivo->proximoDia(); @endphp
        <div class="flex flex-col items-center gap-1.5 flex-1">
          ...
          <div class="w-full">
            ...
            <div class="w-10 h-10 rounded-xl mx-auto flex items-center justify-center
                 ...
                 style="
                   @if($diaReg)
                     background:var(--color-verde);color:white;box-shadow:0 2px 8px rgba(44,95,46,.25);
                   @elseif($esSiguiente)
                     background:var(--color-trigo);color:var(--color-corteza);
                     box-shadow:0 0 0 3px rgba(200,169,110,.25),0 2px 8px rgba(200,169,110,.2);
                   @else
                     background:rgba(107,66,38,.08);color:var(--color-corteza);opacity:.4;
                   @endif
                 ">
              @if($diaReg)
            ...
            <div class="text-center">
              <div class="text-xs font-medium"
                   style="color:var(--color-corteza);opacity:{{ $diaReg ? '.7' : ($esSiguiente ? '.9' : '.3') }}">
                Día {{ $d }}
              </div>
              @if($diaReg)
                <div class="font-mono text-xs mt-0.5" style="color:var(--color-verde);opacity:.7">
                  pH {{ number_format($diaReg->ph ?? 0, 1) }}
                </div>
              @elseif($esSiguiente)
                <div class="text-xs mt-0.5" style="color:var(--color-trigo);opacity:.8">Pendiente</div>
              @endif
            </div>
          </div>
        ...
        {{-- Línea conectora --}}
        @if($d < 5)
          <div class="shrink-0 mt-5" style="width:16px;height:1px;
      ...
      {{-- Conector antes de Pan --}}
      <div class="shrink-0 mt-5" style="width:16px;height:1px;
           background:{{ $procesoActivo->dias->count() >= 5 ? 'rgba(44,95,46,.3)' : 'rgba(107,66,38,.1)' }}">
      </div>

      {{-- Pan --}}
      @php $tienePan = $procesoActivo->panes->isNotEmpty(); $panActivo = !$tienePan && $procesoActivo->dias->count() >= 5; @endphp
      <div class="flex flex-col items-center gap-1.5" style="width:72px;flex-shrink:0">
        <div class="w-full h-10 rounded-xl flex items-center justify-center gap-1 text-xs font-medium transition-all"
             style="
               @if($tienePan)
                 background:var(--color-verde);color:white;box-shadow:0 2px 8px rgba(44,95,46,.25);
               @elseif($panActivo)
               ...
               @else
                 background:rgba(107,66,38,.08);color:var(--color-corteza);opacity:.35;
               @endif
             ">
          @if($tienePan)
        ...
        <div class="text-xs text-center"
             style="color:var(--color-corteza);opacity:{{ $tienePan ? '.7' : ($panActivo ? '.9' : '.3') }}">
          Elaboración
        </div>
      </div>
    ...
    {{-- ── ACCIÓN SIGUIENTE ── --}}
    <div class="mt-6 pt-5 border-t flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-3"
         style="border-color:rgba(107,66,38,.1)">
      <div>
        <div class="text-xs font-mono uppercase tracking-widest mb-0.5"
             style="color:var(--color-corteza);opacity:.35">Siguiente paso</div>
...
<div class="pan-card mb-5">
  <div class="flex flex-col items-center justify-center py-14 px-6 text-center">
    ...
    <h3 class="font-display font-semibold mb-2" style="font-size:1.15rem;color:var(--color-corteza)">
      Sin proceso activo
    </h3>
    <p class="text-sm mb-6 max-w-xs" style="color:var(--color-corteza);opacity:.4;line-height:1.65">
...
<div class="pan-card p-5">
  <div class="flex items-center gap-2 mb-4">
    ...
    <h3 class="text-sm font-semibold" style="color:var(--color-corteza)">Datos de la panadería</h3>
  </div>
...
<style>
  ...
  .pan-btn-ghost {
    background: white; color: var(--color-corteza);
    border: 1px solid rgba(107,66,38,.15); opacity: .7;
  }
  .pan-btn-ghost:hover { opacity: 1; box-shadow: 0 2px 8px rgba(107,66,38,.1); }
  .pan-btn-primary {
    background: var(--color-verde); color: white;
    box-shadow: 0 2px 10px rgba(44,95,46,.22);
  }
  .pan-btn-primary:hover { opacity: .92; box-shadow: 0 4px 16px rgba(44,95,46,.3); }
  .pan-btn-verde {
    background: var(--color-verde-dark); color: white;
    box-shadow: 0 2px 10px rgba(30,69,32,.2);
  }
  ...
  .pan-badge-amber { background: rgba(217,119,6,.1); color: #b45309; }
  .pan-badge-green { background: rgba(44,95,46,.1);  color: var(--color-verde); }

  .pan-dt {
    font-family: 'DM Mono', monospace;
    font-size: .6rem; letter-spacing: .12em; text-transform: uppercase;
    color: var(--color-corteza); opacity: .38; margin-bottom: .2rem; display: block;
  }
  .pan-dd {
    font-size: .85rem; color: var(--color-corteza); opacity: .78; line-height: 1.4;
  }
